(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        let mustDieArr = [ "rusnya" ];
        function mustDieAdd(arr, ...elem) {
            elem.forEach((element => {
                arr.push(element);
            }));
            return arr;
        }
        mustDieAdd(mustDieArr, "trump", "musk", "dj vance", "marco rubio", "witkoff");
        mustDieAdd(mustDieArr, "once more time trump");
        mustDieAdd(mustDieArr, "stupid trump");
        console.log(`Must die: ${mustDieArr.join(", ")}`);
        document.addEventListener("DOMContentLoaded", (() => {
            function addLettersToCircle() {
                ...
                if (!textElem || !circle) return;
                const text = textElem.textContent.trim().split("");
                ...
                for (let i = 0; i < text.length; i++) {
                    ...
                    const tiltAngle = 90 + angle;
                    letter.style.position = "absolute";
                    letter.style.left = `${x}px`;
                    letter.style.top = `${y}px`;
                    letter.style.transform = `translate(-50%, -50%) rotate(${tiltAngle}deg)`;
                    circle.appendChild(letter);
                }
            }
            addLettersToCircle();
            function animateSvgProgressOnClass(parentSelector, watcherClass, valueId, pathId) {
                const parent = document.querySelector(parentSelector);
                if (!parent) return;
                function setSvgProgress() {
                    const progressElem = document.getElementById(valueId);
                    const progressPath = document.getElementById(pathId);
                    if (!progressElem || !progressPath) return;
                    const percent = parseInt(progressElem.textContent, 10);
                    ...
                    progressPath.style.strokeDashoffset = pathLength * (1 - percent / 100);
                }
                if (parent.classList.contains(watcherClass)) setSvgProgress(); else {
                    const observer = new MutationObserver((mutationsList => {
                        for (const mutation of mutationsList) if (mutation.type === "attributes" && mutation.attributeName === "class" && parent.classList.contains(watcherClass)) {
                            setSvgProgress();
                            observer.disconnect();
                        }
                    }));
